<script lang="ts">
	import type { HexSpirit } from '$lib/data/types';

	let { heroSpirits, onNavigate } = $props<{
		heroSpirits: HexSpirit[];
		onNavigate: (id: string) => void;
	}>();

	const hexItemIndices = Array.from({ length: 7 }, (_, index) => index);
</script>

<header class="hero">
	<!-- Floating Wispy Spirits Background -->
	<div class="floating-spirits-container">
		{#each heroSpirits as spirit, i (spirit.id)}
			{@const seed = i * 7919}
			{@const randX = ((seed * 13) % 80) + 5}
			{@const randY = ((seed * 17) % 70) + 10}
			<div
				class="floating-spirit"
				style="
					--start-x: {randX}%;
					--start-y: {randY}%;
				"
			>
				<img src={spirit.image_url} alt="" class="spirit-ghost" loading="lazy" />
			</div>
		{/each}
	</div>

	<div class="hero-bg-slashes">
		<div class="bg-slash bg-slash-1"></div>
		<div class="bg-slash bg-slash-2"></div>
		<div class="bg-slash bg-slash-3"></div>
	</div>
	<div class="hero-content">
		<div class="hero-badge">
			<span class="badge-slash">//</span> NEW PLAYER INTRO
		</div>
		<h1 class="hero-title">
			<span class="title-line">ARC</span>
			<span class="title-line accent">SPIRITS</span>
		</h1>
		<p class="hero-subtitle">TABLETOP SIMULATOR EDITION</p>
		<p class="hero-tagline">
			A simultaneous, map-driven tableau-builder where you program destinations, resolve fights, and
			grow your spirit engine.
		</p>
		<div class="hero-cta">
			<button class="cta-button primary" onclick={() => onNavigate('glance')}>
				<span class="btn-slash">//</span> Start Reading
			</button>
			<a href="#full-rules" class="cta-button secondary">Full Rules Reference</a>
		</div>
		<div class="hero-social">
			<a href="https://discord.gg/arcspirits" target="_blank" rel="noopener noreferrer" class="social-link" aria-label="Join our Discord">
				<svg viewBox="0 0 24 24" fill="currentColor" width="24" height="24">
					<path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028 14.09 14.09 0 0 0 1.226-1.994.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z"/>
				</svg>
				<span>Discord</span>
			</a>
			<a href="https://steamcommunity.com/sharedfiles/filedetails/?id=YOUR_WORKSHOP_ID" target="_blank" rel="noopener noreferrer" class="social-link" aria-label="Steam Workshop">
				<svg viewBox="0 0 24 24" fill="currentColor" width="24" height="24">
					<path d="M12 0C5.37 0 0 5.37 0 12c0 5.084 3.163 9.426 7.627 11.174l3.007-4.238c-.037-.002-.073-.007-.11-.007-2.21 0-4-1.79-4-4s1.79-4 4-4c.93 0 1.785.32 2.463.854l4.063-1.51c.047-.018.096-.03.146-.03.265 0 .48.215.48.48v6.277c0 .265-.215.48-.48.48-.05 0-.099-.012-.146-.03l-4.063-1.51A3.98 3.98 0 0 1 10.524 17c-.037 0-.073-.005-.11-.007l3.007 4.238C17.837 21.426 21 17.084 21 12c0-6.63-5.37-12-12-12zm0 3.75c4.556 0 8.25 3.694 8.25 8.25s-3.694 8.25-8.25 8.25S3.75 16.556 3.75 12 7.444 3.75 12 3.75z"/>
				</svg>
				<span>Steam Workshop</span>
			</a>
			<a href="https://instagram.com/arcspirits" target="_blank" rel="noopener noreferrer" class="social-link" aria-label="Follow on Instagram">
				<svg viewBox="0 0 24 24" fill="currentColor" width="24" height="24">
					<path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z"/>
				</svg>
				<span>Instagram</span>
			</a>
		</div>
	</div>
	<div class="hero-hex-cluster">
		{#each hexItemIndices as i (i)}
			<div class="hex-item" style="--i: {i}"></div>
		{/each}
	</div>
</header>
